...
:::message
書き込みモードで起動して書き込みを行った場合、書き込みが完了しても自動リセットは掛かりません。プログラムを実行するには、Resetボタンを押して手動でリセットする必要があります。
:::
...
なお、`setFont()` でカスタムフォントを指定した場合と、デフォルトフォントを使った場合では `setCursor()` 関数の動作が変わる点に注意が必要です。
...
### カスタムフォントの調整 - 文字送り幅の調整

今回のAnonymous Proは等幅フォントですので、文字送り幅(`xAdvance`)はすべて同じ値となっているのが想定です。
しかし生成したヘッダファイルを見たところ、ほとんどの文字の送り幅は9でしたが、一部の文字は8となっていました。変換プログラムの計算誤差によるものではないかと思われますが、送り幅がずれていると表示位置もずれてしまい、都合がよくありません。
...
const GFXglyph Anonymous_Pro8pt7bGlyphs[] PROGMEM = {
  ...
  {   273,   7,  10,   8,    0,   -9 },   // 0x47 'G'
  {   282,   7,  10,   9,    0,   -9 },   // 0x48 'H'
    ...
  {   385,   7,  10,   9,    0,   -9 },   // 0x53 'S'
  {   394,   7,  10,   8,    0,   -9 },   // 0x54 'T'
  {   403,   7,  10,   9,    0,   -9 },   // 0x55 'U'
    ...
  {   586,   5,  10,   9,    1,   -9 },   // 0x6C 'l'
  {   593,   7,   7,   8,    0,   -6 },   // 0x6D 'm'
  {   600,   7,   7,   9,    0,   -6 },   // 0x6E 'n'
```

9,G,T,mの4文字の文字送り幅が8になっていましたので、9に変更します。
これで、正しく等幅フォントとして動作するようになります。


### カスタムフォントの調整 - 字形の調整

一般に、TTFフォントなどのベクトルフォントを小さいサイズで表示しようとすると、字形が崩れる場合があります。
...
グリフのサイズは8x10であることが分かりますので、ビットマップデータを2進数に変換して、1行を8ビットで成形すると以下のようになります。少し左に傾いたような字形になっていることが分かるでしょう。
...
次は3の字形を例にします。まずは以下のコマンドで3のグリフ情報を抽出します。
...
```C
const uint8_t Anonymous_Pro8pt7bBitmaps[] PROGMEM = {
  0x3D, 0x86, 0x08, 0x11, 0xC0, 0xC0, 0xC1, 0xC6, 0x78 };

const GFXglyph Anonymous_Pro8pt7bGlyphs[] PROGMEM = {
  {     0,   7,  10,   9,    0,   -9 } }; // 0x33 '3'
```

グリフのサイズは7x10なので、少し面倒です。
まずは2進数に直します。
...
```C
const uint8_t Anonymous_Pro8pt7bBitmaps[] PROGMEM = {
  0x3C, 0x86, 0x08, 0x11, 0xC0, 0x40, 0xC1, 0x42, 0x78 };
```
...
残りの 6,8,9,m の字形についても同じように調整します。
...
3,6,9の字形について、まだ若干不満があったので、もう少し改善してみることにしました。

同じような丸みを帯びた文字でも、0,5,8についてはあまり不満が無かったので比較してみたところ、0,5,8は8x10のビットマップになっており、3,6,9は7x10のビットマップになっていました。

そこで、3,6,9の字形も8x10で作り直してみることにしました。

上記の7x10で作成した3の字形は以下のようになっていました。
...
```C
const uint8_t Anonymous_Pro8pt7bBitmaps[] PROGMEM = {
  0x3C, 0x42, 0x81, 0x02, 0x1C, 0x02, 0x01, 0x81, 0x42, 0x3C };
```
...
const GFXglyph Anonymous_Pro8pt7bGlyphs[] PROGMEM = {
  ...
  {   137,   8,  10,   9,    0,   -9 },   // 0x35 '5'
    ...
```
'3' の `width` を7から8に変更しています。さらにビットマップのサイズが1byte増加したので、'4' 以降のすべての文字の `bitmapOffset` を1ずつ増やしています。

6,9についても同じように8x10に変更したところ、字形の不自然さはほぼ解消されました。
...
あとは、上記の手順でデータを結合します。16ポイントについては、字形の調整は行いませんでした。また、文字送り幅については1文字だけ値の違うものがあったので、他と同じ値に修正してあります。
...
:::message
2024-03-25 追記  
上記のPRは無事マージされ、1.0.4としてリリースされました。今後は "Ambient ESP32 ESP8266 lib" の1.0.4以降をライブラリマネージャーからインストールすればそのまま使うことができます。
:::
...
チャタリングを除去する（英語では debounce）ために、今回は最初にボタンが押されてから50ms経った時点で、ボタンの状態を確認し、ボタンが押されていれば実際の処理を行うようにしました。
