...
* printf(): "Full x.xk, prints floats"
  printfで浮動小数を扱えるようにするため。(メモリー使用量は増えるが許容範囲内)
...
基準電圧ICの[TL431](https://akizukidenshi.com/catalog/g/g112018/)をPD7ピン(VREFAピン)に入力し、そのADC値を取得し、そこから電源電圧を逆算しています。
...
なお、AVR自身にも1.024V, 2.048V, 4.096V, 2.500Vの電圧リファレンスが内蔵されており、これらを基準にVDDDIV10 (Vddを1/10にした電圧)を測ることでVddを算出することもできます。ただ、この方法の場合、Vddを測定する際には基準電圧を内蔵リファレンスに変更し、その他の電圧を測定する際には基準電圧をVddに戻す手間が発生します。また、実際に測定した値を見た範囲では、TL431を使った方式の方が安定性が高そうでした。
...
待ち時間や電圧の閾値は、試行錯誤した結果の値です。これらの値を使うことで、ほぼ誤動作なく電池の挿入状態を検出できるようになりました。
...
ここまででNiMH充電器&測定器の要素機能の実装がほぼ完了しました。次はこれらを組み合わせて充放電機能を実装していきます。
...
当初よりNiMH充電器&測定器では、内部抵抗の大きくなってしまった電池でも充電電圧を正しく測定できるようにするため、電圧測定時には一時的に充電を停止し、間欠的に充電を行うことを予定していました。

今回の回路は定電流充電を基本としていますが、前述の電池挿入状態の検出機能を組み合わせることにより、図らずも半ば自動的に間欠充電が実装されました。
...
「電池応用ハンドブック」の第3-6章では、急速充電充電時の電池電圧をより正確に測定するために、充電を一時停止して電圧を測定する「ブレイクスルー方式」というものが提案されています。しかし、その後日談として、その方式が既に特許(特公平8-13169)になっていたことが明かされています。出願は1991年ですので、この本が発行された2005年時点ではまだ特許は有効でしたが、2011年には特許は切れています。
...
実際にいくつかの電池で充電を行ってみたところ、電池によりかなり終止電圧が変わる(1.49V ~ 1.51V)ことが判明しました。
...
次に実装することにしたのは、NiCd/NiMH急速充電器で一般的に使用されている-ΔV検出による充電停止です。
...
-ΔV検出処理を開始する1.38Vというのは、充電終盤でグラフの傾きが大きくなり始める付近の電圧を指しています。
SMAを取っているのは、1回の電圧測定では、数mV程度のぶれは頻繁に発生するからです。

なお、-ΔV検出の前に終止電圧監視で充電が停止してしまわないように、終止電圧の設定は1.53Vに上げています。
...
ただ、ここまで劣化した電池であっても、ワイヤレスマウス、時計などの低消費電力機器にはまだまだ使用可能です。(通常の充電器では充電できませんが…)
