...
ここまでで「NiMH充電器&測定器」の要素機能の実装がほぼ完了しました。次はこれらを組み合わせて充放電機能を実装していきます。
...
当初より「NiMH充電器&測定器」では、内部抵抗の大きくなってしまった電池でも充電電圧を正しく測定できるようにするため、電圧測定時には一時的に充電を停止し、間欠的に充電を行うことを予定していました。
