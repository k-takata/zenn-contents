...
トータルの実行時間は1時間10分程度も掛かるうえ、後からGitHub Actionsのワークフローを追加したことでGitHub ActionsとAppVeyorを行ったり来たりする複雑な構成になっていました。
...
1. ワークフローA
   メインのワークフロー
2. ワークフローB
   トリガーイベントに `workflow_dispatch` を設定して、手動実行できるようにしておく。
3. ワークフローC
   トリガーイベントに `workflow_run` を設定して、ワークフローBの実行が終わったら実行されるようにしておく。
...
例えば、あるステップからシェルスクリプトを呼び出したいときに、[msys2/setup-msys2](https://github.com/msys2/setup-msys2)の `msys2` コマンドを介して実行する例を考えてみます。
  ...
  job1:
    ...
    - shell: cmd
      run: |
        ...
        echo msys2 finished.          # ← callを忘れると実行されない
```

`msys2` コマンドは実際にはバッチファイル (`msys2.cmd`) なので、`call` コマンドを忘れると、それ以降のコマンドがスキップされてしまいます。
...
vim-win32-installerのCIを、AppVeyorからGitHub Actionsに移行し、実行時間を大幅に短縮することができました。また、CIの構成を整理し、今後のメンテナンスの手間を減らすことができたと考えています。

また、移行の際のトラブルも記載しました。何かの参考になれば幸いです。
