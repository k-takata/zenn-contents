...
環境変数 `GH_TOKEN` に `GITHUB_TOKEN` を設定して `gh workflow run` コマンドを実行すれば、ワークフローBは実行されますが、ワークフローCは実行されません。Cも実行されるようにするには、`GITHUB_TOKEN` 以外のトークンを使用する必要があります。
